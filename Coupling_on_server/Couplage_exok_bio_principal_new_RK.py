...
x_vals = np.linspace(5, 50, num=8, dtype=int)      # 10 valeurs sur [40, 60]
y_vals = np.linspace(30, 200, num=22, dtype=int)      # 5 valeurs sur [30, 150]

# Produit cartésien
doublets = [(x, y) for x in x_vals for y in y_vals]
...
if task_id < len(doublets): niter1,niter2 = doublets[task_id]
else: raise ValueError("Trop de taches")
...
#        evol_mars = defining_atm(fH, fG, fN, Ptop, Psurf, T_surf,acceleration=4)
#        evol_mars.equilibrate(Fnet_tolerance=0.1, N_timestep_ini=8, N_timestep_max=1000, verbose=False)
#except:
#    try:
#        with io.capture_output() as _:
#            evol_mars = defining_atm(fH, fG, fN, Ptop, Psurf, T_surf,acceleration=3)
#            evol_mars.equilibrate(Fnet_tolerance=0.1, N_timestep_ini=8, N_timestep_max=1000, verbose=False)
#    except:
#        try: 
#            with io.capture_output() as _:
#                evol_mars = defining_atm(fH, fG, fN, Ptop, Psurf, T_surf,acceleration=2)
#                evol_mars.equilibrate(Fnet_tolerance=0.1, N_timestep_ini=8, N_timestep_max=1000, verbose=False)
#        except:
with io.capture_output() as _:
...
if Teq > 271:
    for i in range(niter1+niter2):
        ...
        Bio, Medium,Atmo, t,flux,flux_times,Pression=system_evolution_RK(Psurf, Teq, 550*Mars_surface,1/11 * fraction_ocean, 1e6, Mars_surface, 
                 ...
                 concentration_ini=(dico_ini['cH'],dico_ini['cC'],dico_ini['cG'],dico_ini['cN']),
                                NC_0=dico_ini['NC'], rtol=1e-7, atol=1e-20,methode='LSODA',N=int(1e5),firststep=None,minstep=np.nan)
