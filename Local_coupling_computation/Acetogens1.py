...
Catabolism = np.array([0,0.5,0,0,0,-1,0.25,0,0,0,0,0,0])    # Catabolism stoichiometry
...
def DeltaGcat(T,CO,C,CH3COOH):
    """
    Computes the catabolic gibbs energy for the H2 autotrophs
    """
    global deltaG0Cat
    global deltaH0Cat
    return(DeltaG(T,deltaG0Cat,[CO,C,CH3COOH],[-1,0.5,0.25],deltaH0Cat))
...
def QCat(deltaGcat,CO,qmax,ks):
    """
    Computes the rate at which catabolic reaction occurs 
    """
    if deltaGcat < 0:
        return(Monod(qmax,ks,Slim([CO],[-2])))
    else:
        return(0)
...
def Gamma(thresh,slope,gmax,x,Qc):
    ...
    """
    if x > 2*Qc:
        gamma = 1/(1+np.exp(-slope*(np.log10((x-2*Qc)/Qc))))
    else:
        gamma = 0
    return(gamma)
