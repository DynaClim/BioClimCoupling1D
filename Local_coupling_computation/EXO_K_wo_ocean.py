...
def compute_R_over_cp(fN2, fCO2, fCH4, fH2):
    """
    Computes R/cp for a gas mixture given its molar fractions.

    Arguments:
        fN2, fCO2, fCH4, fH2 : molar fractions of N₂, CO₂, CH₄, and H₂

    Returns:
        R/cp (dimensionless)
    """

    # Check that the sum of the fractions is close to 1
    total_fraction = fN2 + fCO2 + fCH4 + fH2
    if abs(total_fraction - 1.0) > 1e-6:
        raise ValueError("The sum of molar fractions must be equal to 1.")

    # Gamma values for each gas
    gamma_N2 = 1.4
    gamma_CO2 = 1.3
    gamma_CH4 = 1.32
    gamma_H2 = 1.41

    # Compute R/cp for each gas
    Rcp_N2 = 1 - 1 / gamma_N2
